<?php
class DataTypeListTests extends PHPUnit_Framework_TestCase
{
	...
	{
		$result = self::$_pbc->query('Alter TABLE '.self::$_tableName. ' ADD fld_list_int list<int>', \McFrazier\PhpBinaryCql\CqlConstants::QUERY_CONSISTENCY_ONE);
		...
		$result = self::$_pbc->query('UPDATE '.self::$_tableName.' SET fld_list_int = [0,1,2,3,4] WHERE row_key = \''.self::$_rowKey.'\'', \McFrazier\PhpBinaryCql\CqlConstants::QUERY_CONSISTENCY_ONE);
		$this->assertEquals('OK',$result->getData()->status);
	
		// select data
		$result = self::$_pbc->query('select fld_list_int from '.self::$_tableName.' WHERE row_key=\''.self::$_rowKey.'\'', \McFrazier\PhpBinaryCql\CqlConstants::QUERY_CONSISTENCY_ONE);
		
		// check column spec for correct column type which is list
		$this->assertEquals(\McFrazier\PhpBinaryCql\CqlConstants::COLUMN_TYPE_OPTION_LIST,$result->getData()->rowMetadata->columnSpec[0]->optionId);
		...
		$this->assertTrue(is_array($result->getData()->rowsContent[0]->fld_list_int));
		
		// check column data for correct column type
		$counter = 0;
		foreach($result->getData()->rowsContent[0]->fld_list_int as $item)
		{
	...
	public function testVarcharList()
	{
		...
		$result = self::$_pbc->query('Alter TABLE '.self::$_tableName. ' ADD fld_list_varchar list<varchar>', \McFrazier\PhpBinaryCql\CqlConstants::QUERY_CONSISTENCY_ONE);
		//var_dump($result->getData());
		$this->assertEquals('UPDATED',$result->getData()->changeType);
		$this->assertEquals(self::$_keyspaceName,$result->getData()->affectedKeyspace);
		$this->assertEquals(self::$_tableName, $result->getData()->affectedTable);
		...
		// ipv4 - update inet data
		$result = self::$_pbc->query('UPDATE '.self::$_tableName.' SET fld_list_varchar = [\''.implode("','",$varChar).'\'] WHERE row_key = \''.$rowKey.'\'', \McFrazier\PhpBinaryCql\CqlConstants::QUERY_CONSISTENCY_ONE);
		$this->assertEquals('OK',$result->getData()->status);
	
		// select data
		$result = self::$_pbc->query('select fld_list_varchar from '.self::$_tableName.' WHERE row_key=\''.$rowKey.'\'', \McFrazier\PhpBinaryCql\CqlConstants::QUERY_CONSISTENCY_ONE);
		...
		// should have an array
		$this->assertTrue(is_array($result->getData()->rowsContent[0]->fld_list_varchar));
	
		// check column data for correct column type
		$counter = 0;
		foreach($result->getData()->rowsContent[0]->fld_list_varchar as $item)
		{
			$this->assertSame($varChar[$counter], $item);
			$counter++;
		}
	...
	public function testUuidList()
	{
		...
		$result = self::$_pbc->query('Alter TABLE '.self::$_tableName. ' ADD fld_list_uuid list<uuid>', \McFrazier\PhpBinaryCql\CqlConstants::QUERY_CONSISTENCY_ONE);
		//var_dump($result->getData());
		$this->assertEquals('UPDATED',$result->getData()->changeType);
		$this->assertEquals(self::$_keyspaceName,$result->getData()->affectedKeyspace);
		$this->assertEquals(self::$_tableName, $result->getData()->affectedTable);
		...
		// ipv4 - update inet data
		$result = self::$_pbc->query('UPDATE '.self::$_tableName.' SET fld_list_uuid = ['.implode(',',$uuid).'] WHERE row_key = \''.$rowKey.'\'', \McFrazier\PhpBinaryCql\CqlConstants::QUERY_CONSISTENCY_ONE);
		$this->assertEquals('OK',$result->getData()->status);
	
		// select data
		$result = self::$_pbc->query('select fld_list_uuid from '.self::$_tableName.' WHERE row_key=\''.$rowKey.'\'', \McFrazier\PhpBinaryCql\CqlConstants::QUERY_CONSISTENCY_ONE);
		...
		// should have an array
		$this->assertTrue(is_array($result->getData()->rowsContent[0]->fld_list_uuid));
	
		// check column data for correct column type
		$counter = 0;
		foreach($result->getData()->rowsContent[0]->fld_list_uuid as $item)
		{
			$this->assertSame($uuid[$counter], $item);
			$counter++;
		}
	}
